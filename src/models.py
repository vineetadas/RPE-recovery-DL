...
def twin_cnn(input_img_layer, Conv1, Conv2, Conv3, Conv4, wt):
...
def twin_discriminator(input_shape, init=RandomNormal(stddev=0.02)):
    ...
    feat_left = twin_cnn(left_input, Conv1, Conv2, Conv3, Conv4, wt)
    feat_right = twin_cnn(right_input, Conv1, Conv2, Conv3, Conv4, wt)
...
def cnn_discriminator(image_shape, init=RandomNormal(stddev=0.02)):
    in_src_image = Input(shape=image_shape)

    d = Conv2D(
        16, (4, 4), strides=(2, 2), padding="same", kernel_initializer=init
    )(in_src_image)
...
def encoder_block(
    layer_in, n_filters, batchnorm=True, init=RandomNormal(stddev=0.02)
):
    g = Conv2D(
        n_filters,
        (4, 4),
...
def decoder_block(
    layer_in, skip_in, n_filters, dropout=True, init=RandomNormal(stddev=0.02)
):
...
# define generator
def generator(image_shape, init=RandomNormal(stddev=0.02)):
    ...
    # output
    g = Conv2DTranspose(
        1, (4, 4), strides=(2, 2), padding="same", kernel_initializer=init
    )(d7)
